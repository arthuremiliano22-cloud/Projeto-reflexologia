...
export default function TherapistAdmin({
  ...
  // Sync edit states with props when props fetch completes or updates in real-time
  React.useEffect(() => {
    if (clinicAddress) setTempAddress(clinicAddress);
    if (onlineMeetingLink) setTempMeetingLink(onlineMeetingLink);
    
    if (therapistContact) {
      if (therapistContact.name) setTempName(therapistContact.name);
      if (therapistContact.specialty) setTempSpecialty(therapistContact.specialty);
      if (therapistContact.bio) setTempBio(therapistContact.bio);
      if (therapistContact.phone) setTempPhone(therapistContact.phone);
      if (therapistContact.email) setTempEmail(therapistContact.email);
      if (therapistContact.instagram) setTempInstagram(therapistContact.instagram);
      if (therapistContact.workingHours) setTempWorkingHours(therapistContact.workingHours);
    }

    const ref = services.find(s => s.type === 'REFLEXOLOGY');
    if (ref) {
      setTempReflexologiaPrice(ref.price);
      setTempReflexologiaDuration(ref.durationMinutes);
    }
    const imt = services.find(s => s.type === 'IMT');
    if (imt) {
      setTempImtPrice(imt.price);
      setTempImtDuration(imt.durationMinutes);
    }
  }, [clinicAddress, onlineMeetingLink, therapistContact, services]);
  ...
  // Login handler with Firebase Authentication fallback
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.warn("Inner login failed:", err);
      if (err.code === 'auth/wrong-password') {
        setLoginError('Senha incorreta.');
      } else if (err.code === 'auth/user-not-found') {
        setLoginError('E-mail de acesso não registrado.');
      } else {
      ...
      <div className="flex items-center justify-center p-6 bg-warm-50 border border-warm-200 rounded-3xl h-[520px]" id="admin-login-shield">
        ...
        >
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            {loginError && (
              <div className="text-xs font-semibold text-red-800 bg-red-50 border border-red-200 rounded-xl p-3">
                ⚠️ {loginError}
              </div>
            )}
            ...
            <button 
              ...
              className="w-full bg-terapia-700 hover:bg-terapia-700/90 text-white font-bold py-3 rounded-xl text-sm transition-all flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
            >
              <Lock className="w-4 h-4 animate-pulse" />
              <span>{isLoggingIn ? 'Entrando...' : 'Entrar na minha Agenda'}</span>
            </button>
          </form>
        </motion.div>
      </div>
    ...
    <div className="bg-[#FAF9F6]/90 border border-warm-200 rounded-[28px] shadow-sm p-5 sm:p-7 lg:p-9 space-y-6" id="therapist-admin-container">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-warm-200 pb-5">
        <div>
          ...
          <h2 className="text-2xl font-serif font-bold text-warm-950 tracking-tight mt-2.5 flex flex-wrap items-center gap-2">
            Workspace Rosani Halmenschlager
      ...
      {activeTab === 'CONFIGS' && (
        <form onSubmit={(e) => {
          ...
          // Save therapist contact details, clinic details, and custom services pricing
          onSaveAllConfigs(
            {
              name: tempName,
            ...
            tempAddress,
            tempMeetingLink,
            Number(tempReflexologiaPrice),
            Number(tempReflexologiaDuration),
            Number(tempImtPrice),
            Number(tempImtDuration)
          );
          
          setConfigsSuccessMsg("Configurações gerais e dados do perfil médico atualizados com sucesso!");
          setTimeout(() => {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder="Ex: Rosani Halmenschlager"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-warm-50/70 border border-warm-200 rounded-xl px-3.5 py-3 text-xs text-warm-955 focus:outline-none focus:border-terapia-700 focus:ring-1 focus:ring-terapia-200 transition-all font-sans shadow-3xs"
                    placeholder="Ex: Reflexoterapeuta Integrativa & Corporal"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-warm-50/70 border border-warm-200 rounded-xl px-3.5 py-3 text-xs text-warm-955 focus:outline-none focus:border-terapia-700 focus:ring-1 focus:ring-terapia-200 transition-all font-sans shadow-3xs"
                    placeholder="Ex: rosani@reflexoterapia.com.br"
                  />
                </div>
      ...
      {/* Floating Balloon Toast Notification */}
      <AnimatePresence>
        {configsSuccessMsg && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-[9999] max-w-sm sm:max-w-md w-[calc(100vw-3rem)] bg-white border-2 border-emerald-500 rounded-2xl shadow-xl p-4 flex items-start gap-3.5 ring-4 ring-emerald-50"
            id="floating-save-toast"
          >
            <div className="bg-emerald-50 border border-emerald-250 text-emerald-700 p-2 rounded-xl shrink-0 flex items-center justify-center">
              <Check className="w-5 h-5 stroke-[3]" />
            </div>
            
            <div className="flex-1 space-y-1 pr-1">
              <h4 className="font-serif font-black text-warm-950 text-sm leading-none flex items-center gap-1.5 pt-0.5">
                <span>Informações Salvas!</span>
                <span className="inline-block w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
              </h4>
              <p className="text-[11px] text-warm-850 font-semibold leading-relaxed">
                As alterações estruturais de valores, duração e dados do local físico foram salvas e publicadas no Portal de Agendamento.
              </p>
              <div className="flex items-center gap-1.5 pt-0.5 font-sans text-[9px] font-black uppercase tracking-wider text-warm-400">
                <Clock className="w-3 h-3 text-warm-450" />
                <span>Auto-ocultar em 5s</span>
              </div>
            </div>

            <button
              type="button"
              onClick={() => setConfigsSuccessMsg(null)}
              className="p-1.5 rounded-lg border border-warm-250 hover:bg-warm-100 text-warm-800 hover:text-warm-950 cursor-pointer transition-all shrink-0 hover:scale-105 active:scale-95 flex items-center justify-center"
              title="Fechar notificação"
              id="close-floating-toast"
            >
